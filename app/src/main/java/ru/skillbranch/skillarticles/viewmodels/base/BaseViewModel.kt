...
abstract class BaseViewModel<T : IViewModelState>(
    ...
    /***
     * функция для навигации пользователя (переход обрабатывается только один раз)
     * соответсвенно при изменении конфигурации и пересоздании Activity навигация не будет вызвано
     * повторно
     */
    ...
    /***
     * более компактная форма записи observe() метода LiveData вызывает лямбда выражение обработчик
     * только в том случае если навигация не была уже обработанно ранее,
     * реализует данное поведение с помощью EventObserver
     */
